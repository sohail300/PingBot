...
@target_router.post("/list", response_model=List[TargetListResponse], status_code=status.HTTP_200_OK)
def list_targets(user: get_current_user_dependency, db: db_dependency):
    return TargetService.list_targets(user, db)


@target_router.post("/dashboard-stats", response_model=Dict, status_code=status.HTTP_200_OK)
def dashboard_stats(user: get_current_user_dependency, db: db_dependency):
    return TargetService.dashboard_stats(user, db)


@target_router.post("/toggle", response_model=Dict, status_code=status.HTTP_201_CREATED)
...
@target_router.post("/delete", response_model=Dict, status_code=status.HTTP_201_CREATED)
